...
moose1_amt<- make_track(tbl= moose1, #make_track converts non-spatial data into a movement track object
                         ...
                         id= id, 
                         crs= 3978)
#4. KDE berechnen FÜR EINZELNES TIER
...
moose_amt<- make_track(tbl= moose_ng, 
                         ...
                         id= id, 
                         crs= 3978)
#6. KDE Berechnung GESAMT
kde_moose_95 <- amt::hr_kde(x = moose_amt, levels = c(0.95))
plot(kde_moose_95)

#6. KDE für jedes Tier einzeln ausgeben lassen, sortiert nach Geschlecht
#-> Spalte mit Sex Info wieder an amt objekt anhängen
moose_amt <- moose_amt %>%
  left_join(moose_ng %>% select(id, animal.sex) %>% distinct(), by = "id")
...
#Flächen für alle individuen anzeigen lassen
kde_moose1 <- kde_results[kde_results$id == "1", ]
