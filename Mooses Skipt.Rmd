...
```
```{r}
rsf_df_2 <- bind_rows(env_all_list)
rsf_df_2$case_ <- as.integer(rsf_df$case_) # TRUE/FALSE → 1/0
rsf_df_2$landcover <- as.factor(rsf_df$landcover)

# Anwednung GLMM
library(lme4)
model_glmm_2 <- glmer(case_ ~ elevation + landcover + (1 | id),
                    data = rsf_df_2,
                    family = binomial)
...
rsf_stack_2 <- rsf_stack  # Originaldaten beibehalten

# Sicherstellen, dass landcover im Raster als Faktor mit passenden Levels vorliegt
# 3.1 Hole Modell-Levels
model_levels <- levels(rsf_df_2$landcover)

# 3.2 Erstelle Lookup-Tabelle für Raster-Faktor
landcover_levels <- data.frame(
  value = as.integer(model_levels),  # z. B. 1, 2, 5, ...
  label = model_levels               # z. B. "1", "2", "5", ...
)

# 3.3 Raster-Faktor umwandeln und Levels setzen
rsf_stack_2$landcover <- as.factor(rsf_stack_2$landcover)
levels(rsf_stack_2$landcover) <- list(landcover_levels)

# 3.4 Werte im Raster prüfen – falls dort Kategorien vorkommen, die im Modell nicht enthalten sind, auf NA setzen
known_vals <- as.integer(model_levels)
rsf_stack_2$landcover[!(rsf_stack_2$landcover %in% known_vals)] <- NA

# ------------------------------
# 4. Vorhersage mit GLMM 2
# ------------------------------
moose_pred_logit_2 <- terra::predict(
  object = rsf_stack_2,
  model = model_glmm_2,
  type = "response"
)
...
```{r}
moose_prob  <- function(var2) {1/ (1 + exp(x = -var2))}  # inverse logit

moose_pred_logit <- terra::predict(object = rsf_stack, model = model_glmm, type = "response")


```

## the prediction is made on logit scale
moose_pred_logit <- terra::predict(object = rsf_stack, 
                                model = model_glmm)
...
ggplot(ranef_df, aes(x = reorder(id, intercept), y = intercept)) +
  ...
  labs(x = "Individuum (ID)", y = "Zufälliger Interzept",
