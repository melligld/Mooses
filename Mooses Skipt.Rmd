...
sex<- read.csv("/Users/melan/OneDrive/Dokumente/Uni/Mooses movement/Mooses R/data/Peters_Hebblewhite_Alberta-BC_Moose-reference-data.csv")
...
moose_sf <- st_as_sf(moose_data, coords = c("lon", "lat"), crs = 4326)
...
```
```{r}
#Home Range Size mit dem Minimum Convex Polygon (CMP) für Elch 3 berechnen
...
#CSV-Dateien laden und zusammenfügen (wie oben bereits)
csv_files <- list.files("data", pattern = "\\.csv$", full.names = TRUE)
moose_data <- purrr::map_dfr(csv_files, read.csv)


#Spalten umbenennen & Zeitstempel vorbereiten
moose_data <- moose_data %>%
  rename(
    lon = location.long,
    lat = location.lat,
  ...
  filter(!is.na(lon), !is.na(lat), !is.na(timestamp)) #entfernen von NA

#Als sf-Objekt speichern und in Meter umwandeln
moose_sf <- st_as_sf(moose_data, coords = c("lon", "lat"), crs = 4326)
moose_sf <- st_transform(moose_sf, 3035) #Ausgabe in meter


#Koordinaten extrahieren für amt
moose_sf$X <- st_coordinates(moose_sf)[,1]
moose_sf$Y <- st_coordinates(moose_sf)[,2]


#In amt-Track umwandeln
moose_df <- moose_sf %>% st_drop_geometry()

moose_track <- make_track(
  tbl = moose_df,
  .x = X, .y = Y, .t = timestamp,
  id = id,
  crs = 3035
...
#Home Range Size berechnen für iD 3
moose_id3 <- moose_track %>% filter(id == 3)

#MCP (Minimum Convex Polygon)
mcp_95 <- hr_mcp(moose_id3, levels = 0.95)


#Plotten der Home Range
plot(mcp_95, main = "MCP 95% – Elch 3")


#Fläche in Quadratkilometern ändern
mcp_area <- hr_area(mcp_95)$area / 1e6  # m² -> km²

cat("📐 MCP-Fläche:", round(mcp_area, 2), "km²\n")
```
